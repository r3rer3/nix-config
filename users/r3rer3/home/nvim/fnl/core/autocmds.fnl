...
; Markdown
(add-new :markdown [:*.md :*.markdown] [:BufRead :BufNewFile]
         (fn []
           (set vim.wo.wrap true)))

; Makefile
(add-new :make :Makefile [:BufRead :BufNewFile]
         (fn []
...
(let [ag-ocaml (add-new :ocaml :ocaml)]
  (update-ag ag-ocaml :*.mli [:BufRead :BufNewFile]
             (fn []
               (set vim.bo.filetype :ocamlinterface)))
  (update-ag ag-ocaml :*.mly [:BufRead :BufNewFile]
             (fn []
               (set vim.bo.filetype :menhir)))
  (update-ag ag-ocaml :*.mll [:BufRead :BufNewFile]
             (fn []
             ...
             (when (not= nil (next buf-info))
               (let [win-id (. buf-info 1)
                 ...
                 (if (= win-info.loclist 1)
                     (do
                       (map :n :<localleader>j (fn [] (nvim_command :lnext))
                            {:desc "Goes to next item on Loclist"})
                       (map :n :<localleader>k (fn [] (nvim_command :lprev))
                            {:desc "Goes to previous item on Loclist"})
                       (map :n :<C-c> (fn [] (nvim_command :lclose))
                            {:desc "Closes Loclist"}))
                     (do
                       (map :n :<localleader>j (fn [] (nvim_command :cn))
                            {:desc "Goes to next item on Quickfix"})
                       (map :n :<localleader>k (fn [] (nvim_command :cp))
                            {:desc "Goes to previous item on Quickfix"})
                       (map :n :<C-c> (fn [] (nvim_command :cclose))
                            {:desc "Closes Quickfix"}))))))))
